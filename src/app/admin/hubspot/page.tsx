...
export default function HubSpotAdminPage() {
  ...
  const handleTriggerSync = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setSyncFeedback(`Successfully synchronized ${data.summary.contactsCount} contacts, ${data.summary.dealsCount} deals, and ${data.summary.companiesCount} companies from HubSpot.`);
        loadData();
      } else {
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-secondary/30 bg-secondary/10 px-3 py-1 text-xs font-medium text-secondary mb-2">
            ...
            <span>HubSpot CRM Integration Suite (Portal: 27244747)</span>
          </div>
          <h1 className="font-heading text-3xl font-bold text-white tracking-tight">
            HubSpot CRM & Services Explorer
          </h1>
          <p className="text-neutral-400 text-sm mt-1">
            Real-time synchronization for contacts, sponsorship deals, company records, and service requests.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{syncing ? "Synchronizing CRM..." : "Sync All HubSpot Data"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <p className="text-[11px] text-neutral-400">Members, partners & leads</p>
        </div>
        ...
        >
          ...
          <p className="text-[11px] text-neutral-400">Ecosystem organizations</p>
        </div>
        ...
        >
          <div className="flex items-center justify-between text-neutral-400">
            <span className="text-xs uppercase font-bold tracking-wider">Sync Status</span>
            <RefreshCw className="size-4 text-secondary" />
          </div>
          <p className="text-3xl font-bold text-green-400 font-heading">Active</p>
          <p className="text-[11px] text-neutral-400">Auto-pull from EU1</p>
        </div>
      </div>
      ...
      {activeTab === "contacts" && (
        <div className="grid gap-6 lg:grid-cols-12">
          <div className={`${selectedContact ? "lg:col-span-7" : "lg:col-span-12"} rounded-3xl border border-white/10 bg-white/5 overflow-hidden`}>
            ...
            <div className="lg:col-span-5 rounded-3xl border border-white/10 bg-white/5 p-6 space-y-6">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="p-3 rounded-2xl bg-black/40 border border-white/10 space-y-1">
                  ...
                  <p className="text-white capitalize">{selectedContact.leadSource || "Website Onboarding Form"}</p>
                </div>

                <div className="p-3 rounded-2xl bg-black/40 border border-white/10 space-y-1">
                  <p className="text-[10px] uppercase text-neutral-400 font-semibold">Last Modified</p>
